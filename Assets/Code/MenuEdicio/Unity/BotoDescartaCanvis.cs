...
public class BotoDescartaCanvis : MonoBehaviour {
	...
	void Start () {
		textura = Resources.Load("boto_descartar_canvis") as Texture2D;
	}
	...
	void OnGUI(){
		Rect rect = new Rect(0.4f*Camera.mainCamera.pixelWidth, 
		...
		if(detectaClick(rect)){
			...
			aO.playClick();
			AccioEdicio a = new AccioNoDesaCanvis();
			a.executarAccio();
		}
	}
